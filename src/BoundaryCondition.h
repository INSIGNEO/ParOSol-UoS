...
public:
  ...
  std::vector<double> LoadedNodes;
    ...
    //! it holds the index of the BC to access it in the displacements array
  std::vector<t_index> Bodyforce_Ind;
  //! FixedNodes hold the value of the BC.
  ...
  void GenerateBC(std::vector<t_boundary_node> &fixed_list,
                  std::vector<t_boundary_node> &loaded_list, std::vector<t_boundary_node> &bodyforce_list) {
    ...
    for (it = bodyforce_list.begin(); it != bodyforce_list.end(); it++) {

      Bodyforce_Ind.push_back(it->first * 3 + it->second.dir);
      Bodyforce.push_back(it->second.disp);
    }
