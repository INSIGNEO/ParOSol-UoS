...
public:
  ...
  void GenerateBC() {
    ...
    std::vector<t_boundary_node> bodyforce_index_list;
    ...
    Distribute_the_BC(_BC_Bodyforce_list);
    GetIndexOfBC(_BC_Bodyforce_list, bodyforce_index_list);
    CheckIndexOfBC(_BC_Bodyforce_list, bodyforce_index_list);
    
    bc.GenerateBC(fixed_index_list, loaded_index_list,bodyforce_index_list);

    Delete_BC_vector(_BC_fixednode_list);
    Delete_BC_vector(_BC_LoadedNode_list);
    Delete_BC_vector(_BC_Bodyforce_list);
    return;
  }
...
public:
  ...
  //Frederik: Added for bodyforce implementation
  std::vector<t_boundary_node> _BC_Bodyforce_list;
...
template <class T> void OctreeGrid<T>::GenerateOctree() {
  ...
  _BC_Bodyforce_list.resize(0);
  bcset.clear();
  it_val = bodyforce_values.begin();
  for (it_coord = bodyforce_coordinates.begin();
       it_coord != bodyforce_coordinates.end();) {
    unsigned short x, y, z, d;
    float disp;
    x = *it_coord;
    ++it_coord;
  ...
  _BC_Bodyforce_list.reserve(bcset.size());
  _BC_Bodyforce_list.insert(_BC_Bodyforce_list.begin(), bcset.begin(),
                             bcset.end());
